...
![캡처.JPG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/f84b0af4-f3ee-4d2d-ac36-9f4598029ed2/캡처.jpg)
...
3. Vue 인스턴스가 초기화 된 직후에 `**beforeCreate**`라는 훅이 실행된다.
  : 아직 인스턴스의 data, event, methods등에 접근할 수 없는 상태여서, DOM에 접근하려고 한다면 에러가 발생된다.
...
7. `**beforeMount`** 훅 실행
  : DOM에 연결되기 바로 직전에 호출되는 훅으로, 템플릿과 런데함수들이 컴파일된 후, 첫 렌더링이 발생되기 직전에 실행된다!
...
11. `**updated**` 훅 실행
  : 컴포넌트 데이터가 변할 때 **재렌더링이 발생된 직후에 실행되는 훅**으로, DOM이 업데이트가 완료된 상태라면 연산과 기능을 동작시킬 수 있다.
