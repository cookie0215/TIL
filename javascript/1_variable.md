...
> 변수 호이스팅  
> : 키워드를 사용해 선언된 식별자들이 런타임(코드가 순차적으로 한줄씩 실행되는 시점을 말함)이전 단계에서 먼저 실행되는 것즉, 코드를 읽을 때 키워드로 선언한 식별자들이 있다면 해당 식별자들을 먼저 읽는다!!   
...
이때 키워드로 선언만한 상태여도 undefined값이 할당되어 있기 때문에  
1번째 console은 undefined가 출력된다.② abc = 100; 이라는 값을 할당한 상태가 실행되면서  
